...
def run_test(filename, k, init='nndsvdar', loss='kullback-leibler', tol=1e-4, max_iter=200, random_state=0):
    ...
    cprint("NMF-CPU:", 'green')
    ts_start = time.time()
    model3.fit(beta=beta)
    ts_end = time.time()
    err3 = nmf.NMF._loss(model3._V, model3.reconstruction, beta=beta, epsilon=EPSILON, square_root=True)
    print(f"NMF_CPU uses {ts_end - ts_start} s, with final loss at {err3} after {model3._iter + 1} iteration(s).")

if __name__ == '__main__':
    cprint("Test 1:", 'yellow')
    run_test("tests/data/nmf_test_1.npy", k=12, loss='frobenius', init='random')

    cprint("Test 2:", 'yellow')
    run_test("tests/data/nmf_test_2.npy", k=12, loss='frobenius', init='random')
